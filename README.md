...
- **Accessibility**: global `focus-visible` ring on every interactive element (one CSS rule, not per-component styling), a skip-to-content link, every image audited for `alt` text, resting-state text contrast fixed (plain-text links on white backgrounds moved from `primary-500` to the darker `primary-600`). Also, this session: **semantic heading order audited and fixed** — `page.tsx` (home), `[category]/page.tsx`, and `tafuta/page.tsx` had no `<h1>` at all (the home page's DOM led with the carousel's `<h2>`); all three now have a real `<h1>` (`sr-only` where a visible duplicate of the existing `SectionHeader` text would look redundant). **Keyboard dismissal** added to `UserMenu.tsx`'s dropdown and `MobileNav.tsx`'s panel — both previously only closed on an outside click or by choosing a link, with no way for a keyboard-only user to back out; both now close on Escape and return focus to the trigger button. **Colorblind check on the Breaking News yellow** (`#FFCC00` on `ink` `#1A1A1A`): computed contrast ratio is 11.5:1 (WCAG AAA), and it's never used to convey "breaking" on its own — every `tone="breaking"` `Badge` usage pairs the color with the literal word "Breaking" as its content, so the status doesn't depend on distinguishing the color at all
...
- **`SECTION_EDITOR` category scope** (`app/(dashboard)/admin/users/page.tsx`'s `CategoryScopeEditor`) — a per-row multi-select on the admin Users page, shown only for `SECTION_EDITOR` accounts, backed by the new `PUT /api/auth/users/:id/categories` endpoint (`user:assign_role`-gated, same as the role dropdown next to it). Full-replace on every checkbox toggle, matching the endpoint's semantics — what's checked is always exactly the server's current scope, never a client-side diff. Previously this required Prisma Studio (`../backend/README.md` §3.5)
...
- Accessibility pass covers contrast/focus/alt-text/skip-link, semantic heading order, keyboard dismissal on the two dropdown/panel widgets, and a colorblind/contrast check on the Breaking News yellow — not done: a full screen-reader walkthrough (NVDA/VoiceOver) and a systematic keyboard-only tab-order pass across every page (only the two components with open/close state were reviewed for keyboard dismissal specifically)
